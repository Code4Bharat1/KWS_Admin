...
const ViewTicket = () => {
  ...
  useEffect(() => {
    ...
    const raffleId = query.get("raffle_id"); // Assuming event_id is passed as 'event_id'

    if (!ticketNo || !raffleId) {
    ...
    const fetchTicket = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_API_KEY}/raffle/ticket/${ticketNo}`,
          {
            params: { raffle_id: raffleId },
          }
        );
  ...
  return (
    <div className="p-6 font-sans bg-gray-50">
      ...
      <div className="mt-6 max-w-md mx-auto p-4 border border-gray-300 rounded-lg bg-white shadow">
        ...
        <p>
          <strong>Civil ID:</strong> {ticket.civil_id}
        </p>
        <p>
          <strong>Amount (KWD):</strong> {ticket.amount_in_kwd}
        </p>
        <p>
          <strong>Time Sold:</strong> {ticket.timeSold}
        </p>
