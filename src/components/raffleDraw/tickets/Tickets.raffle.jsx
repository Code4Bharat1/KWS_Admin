...
const RaffleTickets = () => {
    ...
    const fetchTickets = async () => {
      try {
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_API_KEY}/raffle/tickets/${id}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_BACKEND_API_KEY}/raffle/addticket/${id}`,
        newTicket
      );
  ...
  const handleView = (ticketNo, raffleId) => {
    ...
    router.push(
      `/raffle-draw/tickets/view-ticket?id=${ticketNo}&raffle_id=${raffleId}`
    );
  };
